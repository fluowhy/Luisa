...
class Classifier(object):
    def __init__(self, model, nin, nh, nlayers=1, ks=3, nf=2, do=0.5, device="cpu", pre=False):
        self.model_name = "{}_nin_{}_nh_{}_nlayers_{}_ks_{}_nf_{}_do_{}".format(model, nin, nh, nlayers, ks, nf, do)
        self.device = device
        self.model = IMG2SEQ(nin, nh, nlayers, ks, nf, do)
        self.model.to(device)
        self.best_loss = np.inf
        # save hyperparameters
        names = ["nin", "nh", "nlayers", "ks", "nf", "do"]
        values = [nin, nh, nlayers, ks, nf, do]
        ...
        self.label2onehot = Label2OneHot(len(idx2char))
        # self.weights = torch.ones(len(idx2char), dtype=torch.float, device=device)
        # self.weights[char2idx["<pad>"]] = 0
        # self.ce = torch.nn.CrossEntropyLoss(weight=self.weights, reduction="mean")        
        self.ce = torch.nn.CrossEntropyLoss(ignore_index=char2idx["<pad>"], reduction="mean")
    ...
    def train_model(self, data_loader, clip_value=1.):
        ...
        for idx, batch in tqdm(enumerate(data_loader)):
            ...
            y = y.to(self.device)
            y_oh = self.label2onehot(y)     
            y_pred = self.model(x, y_oh[:, :-1])
            loss = self.ce(y_pred.transpose(1, 2), y[:, 1:])
            loss.backward()
...
if __name__ == "__main__":
    ...
    rnn_model = Classifier(args.model, nin, args.nh, args.nlayers, args.ks, args.nf, args.do, args.d, args.pre)
